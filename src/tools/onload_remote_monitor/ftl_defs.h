...
#define STRUCT_NETIF_STATE_NIC(ctx)                                     \
  ...
  ON_CI_CFG_SEPARATE_UDP_RXQ(                                            \
    FTL_TFIELD_CONSTINT(ctx, ci_uint16,            \
  ...
  FTL_TFIELD_STRUCT(ctx, ci_ni_dllist_t, tx_ready_list, ORM_OUTPUT_STACK) \
...
#define STRUCT_NETIF_IPID_CB(ctx) \
    ...
    FTL_TFIELD_INT(ctx, ci_iptime_t, loop_start_time, ORM_OUTPUT_STACK)     \
    FTL_TFIELD_INT(ctx, ci_iptime_t, low_use_start_time, ORM_OUTPUT_STACK)  \
    FTL_TFIELD_INT(ctx, ci_int32, current_index, ORM_OUTPUT_STACK)          \
    FTL_TFIELD_INT(ctx, ci_int32, max_index, ORM_OUTPUT_STACK)              \
...
#define STRUCT_NETIF_STATE(ctx)                                         \
  ...
  ON_CI_CFG_SEPARATE_UDP_RXQ(                                           \
    FTL_TFIELD_STRUCT(ctx, ef_vi_stats, udp_rxq_vi_stats, ORM_OUTPUT_STACK)\
  )                                                                     \
...
#define STRUCT_TCP(ctx) \
  ...
  ON_CI_CFG_TIMESTAMPING( \
    ...
    FTL_TFIELD_STRUCT(ctx, ci_ni_dllist_link, tx_ready_link, ORM_OUTPUT_EXTRA)    \
