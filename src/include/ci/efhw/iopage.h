...
/* Allocate a single IO page, map it into the specified NIC (nic) and
 * initialise the efhw_iopage structure (p).  The caller must release
 * the page using efhw_iopage_free when it is no longer needed.
 * Returns zero on success or a negative error number on failure. */
extern int efhw_iopage_alloc(struct pci_dev *pci_dev, struct efhw_iopage *p,
			     efhw_iommu_domain *vf_domain,
			     unsigned long iova_base);

/* Free an IO page allocated using efhw_iopage_alloc.  This reverses
 * the effects of efhw_iopage_alloc.  The same parameters must be
 * supplied to both functions. */
extern void efhw_iopage_free(struct pci_dev *pci_dev, struct efhw_iopage *p,
			     efhw_iommu_domain *vf_domain);

/* Map an existing IO page (orig) into another NIC (nic).  A new
 * efhw_iopage structure (p) is initialised to represent the new
 * mapping.  It is invalid to free the original IO page until the
 * effects of this function have been reversed by calling
 * efhw_iopage_unmap.  Returns zero on success or a negative error
 * number on failure. */
...
/* Unmap an IO page (orig) from a NIC (nic).  This reverses the
