...
#if CI_CFG_IPV6
static inline ci_addr_t ipx_hdr_saddr(int af, const ci_ipx_hdr_t* hdr)
{
  if( af == AF_INET6 )
    return CI_ADDR_FROM_IP6(hdr->ip6.saddr);
  else
    return CI_ADDR_FROM_IP4(hdr->ip4.ip_saddr_be32);
}
static inline ci_addr_t ipx_hdr_daddr(int af, const ci_ipx_hdr_t* hdr)
{
  if( af == AF_INET6 )
    return CI_ADDR_FROM_IP6(hdr->ip6.daddr);
  else
    return CI_ADDR_FROM_IP4(hdr->ip4.ip_daddr_be32);
}
#else
/* We use #define to make it lvalue, because our printing primitives break
 * otherwise.  Use ipx_hdr_set_[sd]addr below to set the values. */
#define ipx_hdr_saddr(af, hdr) CI_ADDR_FROM_IP4((hdr)->ip4.ip_saddr_be32)
#define ipx_hdr_daddr(af, hdr) CI_ADDR_FROM_IP4((hdr)->ip4.ip_daddr_be32)
#endif
