...
#define WIDTHCHCK(a, b)  BUG_ON((a) != (b))
#define RANGECHCK(v, WIDTH) \
                BUG_ON(((uint64_t)(v) & ~(__EFVI_MASK64((WIDTH)))) != 0)

/* fields within the first dword */
#define DWCHCK(LBN, WIDTH) BUG_ON(((LBN) < 0) || (((LBN)+(WIDTH)) > 32))

/* fields which span the first and second dwords */
#define LWCHK(LBN, WIDTH)  BUG_ON(WIDTH < __LW(LBN))
