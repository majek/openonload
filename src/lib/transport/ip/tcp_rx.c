...
ci_inline ci_ip_pkt_fmt* ci_netif_pkt_rx_to_tx(ci_netif* ni,
                                               ci_ip_pkt_fmt* pkt) 
{
  if( pkt->refcount == 1 ) {
  ...
  for( ; i < rxp->sack_blocks; i++ ) {
    ...
    */
    if( ! (/*1*/SEQ_LE(start, rxp->ack) | /*2*/SEQ_LT(tcp_snd_nxt(ts), end) |
      ...
      */
      LOG_U(log(LNT_FMT "SACK %d %08x-%08x invalid snd=%08x-%08x",
                                            ...
                                            ciip_tcp_rx_pkt *rxp)
{
  ...
  ci_assert(SEQ_LE(rxp->ack, ts->snd_nxt));
				 ...
				 ciip_tcp_rx_pkt* rxp)
{
  ...
  */
  ...
  ci_assert(SEQ_LE(rxp->ack, tcp_snd_nxt(ts)));
  ci_assert(SEQ_LE(tcp_snd_una(ts), rxp->ack));
  ...
  if( SEQ_LT(tcp_snd_una(ts), rxp->ack) ) {
    ...
    if( OO_SP_IS_NULL(ts->s.local_peer) )
  ...
  ** (potentially) ages. */
  ...
  if( tcp_rcv_usr(ts) == 0 && ci_sock_trylock(netif, &ts->s.b) ) {
    ...
    if( OO_PP_NOT_NULL(ts->recv1_extract) ) {
      /* Receive queue contains at least one buffer, but no data.  So
      ** extract must be same as tail (since can never have empty buffer
      ** after extract. */
      ci_ip_pkt_fmt* pkt;
      ci_assert(OO_PP_EQ(ts->recv1.tail, ts->recv1_extract));
      if( ts->recv1.num > 1 )  ci_tcp_rx_reap_rxq_bufs(netif, ts);
      ci_assert_equal(ts->recv1.num, 1);
      ci_assert(OO_PP_EQ(ts->recv1.head, ts->recv1_extract));
      pkt = PKT_CHK(netif, ts->recv1_extract);
      ci_assert(oo_offbuf_is_empty(&pkt->buf));
      ci_assert(OO_PP_IS_NULL(pkt->next));
  ...
  if( OO_SP_NOT_NULL(ts->s.local_peer) ) {
    /* "Send ACK" to peer */
    ci_tcp_state* peer = ID_TO_TCP(netif, ts->s.local_peer);
    if( peer->s.b.state == CI_TCP_LAST_ACK )
      ci_tcp_drop(netif, peer, 0);
    else {
      ci_tcp_set_slow_state(netif, peer, CI_TCP_FIN_WAIT2);
      if( peer->s.b.sb_aflags & CI_SB_AFLAG_ORPHAN )
        ci_netif_fin_timeout_enter(netif, peer);
    }
  }
...
static int ci_tcp_rx_deliver_rob(ci_netif* netif, ci_tcp_state* ts)
{
  ...
  while( 1 ) {
    LOG_TV(log(LPF "%d ROB deliver packet %d: %x-%x, last_seq = %x, "
               ...
               pkt->pf.tcp_rx.end_seq, last_seq, pkt->pay_len));
                                         ...
                                         ciip_tcp_rx_pkt *rxp)
{
  ...
  */
  ...
  ci_assert(pkt->pay_len);

  oo_offbuf_init(&pkt->buf, CI_TCP_PAYLOAD(tcp), pkt->pay_len);
                                 ...
                                 ciip_tcp_rx_pkt* rxp)
{
  ...
  */
  ...
  if( tsr ) {
    unsigned tsr_rcv_wnd = CI_MIN(tcp_rcv_buff(tls), CI_CFG_TCP_MAX_WINDOW);
                                  ...
                                  ci_ip_cached_hdrs* ipcache)
{
  ...
  */
  ...
  /*! \TODO ?? Packet could contain data.  We currently bin it, so it
  ...
  */
  ...
  if( ci_tcp_listenq_try_promote(netif, tls, tsr, rxp, ipcache) < 0 ) {
  ...
  /*! \TODO enter the close-wait state if FIN received (rfc793 p75) */
 ...
 send_ack:
  ...
  if( pkt != NULL )
    ci_tcp_synrecv_send(netif, tls, tsr, pkt, CI_TCP_FLAG_ACK, NULL);
  return;
 retransmit_synack:
  pkt = ci_netif_pkt_rx_to_tx(netif, pkt);
  if( pkt != NULL )
                             ...
                             ciip_tcp_rx_pkt* rxp, int already_parsed)
{
  ...
  if( pkt->intf_i == OO_INTF_I_LOOPBACK ) {
    local_peer = pkt->pf.lo.tx_sock;
    ci_assert_equal(pkt->pf.lo.rx_sock, tls->s.b.bufid);
  }
  ...
  */
  ...
  {
    int amss = ipcache.mtu - sizeof(ci_tcp_hdr) - sizeof(ci_ip4_hdr);
    amss = ci_tcp_limit_mss(amss, netif->state, __FUNCTION__);
    ipcache.mtu = amss + sizeof(ci_tcp_hdr) + sizeof(ci_ip4_hdr);
  }
#endif
  ...
  */
  ...
  tsr->tcpopts.smss = 536; /* rfc1122 4.2.2.6 until opt sets it */
  ...
  if( OO_SP_NOT_NULL(tsr->local_peer) ) {
    /* Normally, ci_tcp_listenq_try_promote() is called with ACK packet
     * to our SYN-ACK.  With loopback connection, we call
     * ci_tcp_listenq_try_promote() with SYN packet, so let's increase seq
     * number to handle TCP_DEFER_ACCEPT properly. */
    if( tcp->tcp_flags & CI_TCP_FLAG_SYN )
      rxp->seq++;
    ...
    if( ci_tcp_listenq_try_promote(netif, tls, tsr, rxp, &ipcache) < 0 ) {
      ...
      ci_tcp_drop(netif, ID_TO_TCP(netif, tsr->local_peer), EBUSY);
    }
                                ...
                                ciip_tcp_rx_pkt* rxp)
{
  ...
  tcpopts.smss = 536; /* rfc1122 4.2.2.6 */
                                     ...
                                     ciip_tcp_rx_pkt* rxp)
{
  ...
  if( pkt->pay_len <= 0 )
     log("\tshort(pay_len=%d)", pkt->pay_len);
}
#endif
  ...
  if( ts->s.b.state == CI_TCP_TIME_WAIT ) {
    if( tcp->tcp_flags & CI_TCP_FLAG_FIN &&
      ...
      LOG_TC(log(LNT_FMT "dup FIN in TIME_WAIT restarting 2MSL",
                 LNT_PRI_ARGS(netif, ts)));
      ci_netif_timewait_restart(netif, ts);
    }
  }
			   ...
			   ciip_tcp_rx_pkt* rxp)
{
  ...
  */
  pkt->pay_len -= CI_TCP_HDR_LEN(tcp) - ts->incoming_tcp_hdr_len;
  pkt->pf.tcp_rx.end_seq = rxp->seq + pkt->pay_len;
  pkt->pf.tcp_rx.end_seq +=
    (tcp->tcp_flags & CI_TCP_FLAG_SYN) >> CI_TCP_FLAG_SYN_BIT;
  pkt->pf.tcp_rx.end_seq +=
  ...
  */
  if( CI_LIKELY(ts->s.b.state & CI_TCP_STATE_SYNCHRONISED) ) {
    ...
    */
    if( CI_UNLIKELY(SEQ_LT(tcp_snd_nxt(ts), rxp->ack)) )
    ...
    */
        ...
        if( ci_tcp_rx_enqueue_ooo(netif, ts, rxp) ) {
          ...
          if( ts->acks_pending ) {
            ...
            */
            ...
            if( ackpkt )  ci_tcp_send_ack(netif, ts, ackpkt);
          }
          TCP_FORCE_ACK(ts);
        }
      }
    ...
    if( TCP_ACK_FORCED(ts) ) {
      ...
      */
      ...
      if( pkt )  ci_tcp_send_ack(netif, ts, pkt);
    }
    ...
    if(tcp->tcp_flags & CI_TCP_FLAG_SYN) {
      if (SEQ_LT(tcp_rcv_nxt(ts), rxp->seq) ||
        ...
        if( filter_id >= 0 ) {
          ...
          ci_assert(tls->s.b.state == CI_TCP_LISTEN);
          /* handle_rx_listen() expects pkt->pay_len to not be munged,
...
static int ci_tcp_rx_deliver_to_conn(ci_sock_cmn* s, void* opaque_arg)
{
  ...
  CI_IP_SOCK_STATS_ADD_RXBYTE( ts, pkt->pay_len );
  ...
  LOG_TR(log(LNTS_FMT RCV_WND_FMT " snd=%08x-%08x-%08x",
         ...
         log(LNT_FMT "["CI_TCP_FLAGS_FMT"] id=%d s=%08x-%08x a=%08x "
             ...
             rxp->seq + pkt->pay_len - CI_TCP_HDR_LEN(tcp),
             rxp->ack,
   ...
   */
  pkt->pay_len -= ts->incoming_tcp_hdr_len;
  pkt->pf.tcp_rx.end_seq = rxp->seq + pkt->pay_len;
                      ...
                      ci_ip_pkt_fmt* pkt, ci_tcp_hdr* tcp, int ip_paylen)
{
  ...
  rxp.pkt = pkt;
  rxp.tcp = tcp;
  ci_assert_equal(pkt->pay_len, ip_paylen);
  ...
  ci_netif_filter_for_each_match(netif,
                                 ...
                                 ci_tcp_rx_deliver_to_listen, &rxp);
  if(CI_LIKELY( rxp.pkt == NULL ))
    return;
