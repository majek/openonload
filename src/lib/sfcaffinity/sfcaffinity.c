...
static int get_affinity_fd(void)
{
  ...
  if( the_affinity_fd == 0 ) {
    ...
    if( the_affinity_fd >= 0 ) {
      if( 0 /* ?? TODO */ ) {
        /* O_CLOEXEC was only introduced on Linux in 2.6.23.  So just because
         * it is available at compile time does not mean it is supported in
         * the kernel, so we must check.
         */
        rc = fcntl(the_affinity_fd, F_GETFD);
        fcntl(the_affinity_fd, F_SETFD, rc | FD_CLOEXEC);
      }
    }
    else {
