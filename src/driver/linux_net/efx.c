...
void efx_tx_stuck_channel_info(struct efx_nic *efx)
{
	...
	efx_for_each_channel(channel, efx) {
		...
		netif_info(efx, drv, efx->net_dev,
			   "last ev: rx %llu, tx %llu, mcdi %llu, driver %llu, "
			   "drvgen %llu\n", channel->handle_rx_tstamp,
			   channel->handle_tx_tstamp,
			   channel->handle_mcdi_tstamp,
			   channel->handle_driver_tstamp,
			   channel->handle_drvgen_tstamp);
	}
}
