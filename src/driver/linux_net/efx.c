...
static int efx_process_channel(struct efx_channel *channel, int budget)
{
	...
	ci_frc64(&channel->process_evq_complete_tstamp);
	if (spent && efx_channel_has_rx_queue(channel)) {
		struct efx_rx_queue *rx_queue =
			efx_channel_get_rx_queue(channel);
		ci_frc64(&channel->spent_pre_flush_tstamp);
		efx_rx_flush_packet(channel);
		ci_frc64(&channel->spent_post_flush_tstamp);
#if defined(EFX_NOT_UPSTREAM) && defined(EFX_USE_SFC_LRO)
		efx_ssr_end_of_burst(channel);
#endif
		ci_frc64(&channel->push_rx_desc_tstamp);
		efx_fast_push_rx_descriptors(rx_queue, true);
	}
	ci_frc64(&channel->post_spent_tstamp);
	...
	fill_level = efx_tx_queue_fill_level(tx_queue);
	ci_frc64(&channel->tx_restart1_tstamp);
	...
	ci_frc64(&channel->tx_restart2_tstamp);
...
static int efx_poll(struct net_device *dev, int *budget_ret)
{
	...
	*budget_ret -= spent;
#endif
	ci_frc64(&channel->last_channel_process_complete_tstamp);
	if (spent < budget) {
...
#if defined(EFX_NOT_UPSTREAM) && defined(EFX_ENABLE_SFC_XPS)
		if (efx->cpu_channel_map && efx_channel_has_tx_queues(channel))
			efx->cpu_channel_map[cpu] = channel->channel;
#endif
	}
...
void efx_tx_stuck_channel_info(struct efx_nic *efx)
{
	...
	efx_for_each_channel(channel, efx) {
		...
		netif_info(efx, drv, efx->net_dev,
			   "NAPI count: %lu, last TX @ %lu\n",
			   channel->napi_count, channel->last_tx_napi_count);
		netif_info(efx, drv, efx->net_dev,
		...
		netif_info(efx, drv, efx->net_dev,
			   "NAPI poll: process complete %llu\n",
			   channel->last_channel_process_complete_tstamp);
		netif_info(efx, drv, efx->net_dev,
			   "last ev: pre-rx %llu, post-rx %llu\n",
			   channel->handle_rx1_tstamp,
			   channel->handle_rx2_tstamp);
		netif_info(efx, drv, efx->net_dev,
			   "         pre-tx %llu, post-tx %llu\n",
			   channel->handle_tx1_tstamp,
			   channel->handle_tx2_tstamp);
		netif_info(efx, drv, efx->net_dev,
			   "         pre-mcdi %llu, post-mcdi %llu\n",
			   channel->handle_mcdi1_tstamp,
			   channel->handle_mcdi2_tstamp);
		netif_info(efx, drv, efx->net_dev,
			   "process channel: evq-comp %llu\n",
			   channel->process_evq_complete_tstamp);
		netif_info(efx, drv, efx->net_dev,
			   "         pre-flush %llu, post-flush %llu, push-descs %llu, post-spent %llu\n",
			   channel->spent_pre_flush_tstamp,
			   channel->spent_post_flush_tstamp,
			   channel->push_rx_desc_tstamp,
			   channel->post_spent_tstamp);
		netif_info(efx, drv, efx->net_dev,
			   "tx restart:  pre %llu, post %llu\n",
			   channel->tx_restart1_tstamp,
			   channel->tx_restart2_tstamp);
	}
}
