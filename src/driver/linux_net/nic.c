...
static bool efx_disable_port_on_mpe = false;
module_param(efx_disable_port_on_mpe, bool, 0644);
MODULE_PARM_DESC(efx_disable_port_on_mpe, "Disable interface on Memory Parity Error");
...
void efx_magic_event(struct efx_channel *channel, u32 magic)
{
	efx_qword_t event;
...
#ifdef EFX_NOT_UPSTREAM
		if (mem_perr && efx_disable_port_on_mpe)
			efx_schedule_reset(efx, RESET_TYPE_DISABLE);
		else
 ...
 */
int efx_nic_dimension_resources(struct efx_nic *efx, unsigned sram_lim_qw)
{
