...
int efx_farch_ev_process(struct efx_channel *channel, int budget)
{
	...
	for (;;) {
		...
		case FSE_AZ_EV_CODE_DRV_GEN_EV:
			ci_frc64(&channel->handle_drvgen1_tstamp);
			efx_farch_handle_generated_event(channel, &event);
			ci_frc64(&channel->handle_drvgen2_tstamp);
			break;
		case FSE_AZ_EV_CODE_DRIVER_EV:
			ci_frc64(&channel->handle_driver1_tstamp);
			efx_farch_handle_driver_event(channel, &event);
			ci_frc64(&channel->handle_driver2_tstamp);
