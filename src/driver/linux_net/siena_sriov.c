...
MODULE_PARM_DESC(vf_tx_filter,
		 "[SFC9000-family] VF transmit filter mode 0=>never, 1=>auto(default), 2=>always");
#endif
...
MODULE_PARM_DESC(vf_max_tx_channels,
		 "[SFC9000-family] Limit the number of TX channels VFs can use");

static int max_vfs = -1;
module_param(max_vfs, int, 0444);
MODULE_PARM_DESC(max_vfs,
		 "[SFC9000-family] Reduce the number of VFs initialized by the driver");
 ...
 */
static void efx_siena_sriov_reset_vf(struct efx_vf *vf, struct efx_buffer *buffer)
{
	struct efx_nic *efx = vf->efx;
...
static void
efx_siena_sriov_get_channel_name(struct efx_channel *channel, char *buf, size_t len)
{
			    ...
			    const char *buf, size_t count)
{
	struct efx_vf *vf = container_of(attr, struct efx_vf, dev_attr_mac_addr);
	struct efx_nic *efx = vf->efx;
	u8 mac_addr[6] __aligned(2);

	if (!mac_pton(buf, mac_addr))
		return -EINVAL;

	if (!ether_addr_equal(mac_addr, efx->net_dev->dev_addr))
		return -EINVAL;

	mutex_lock(&vf->status_lock);
	ether_addr_copy(vf->addr.mac_addr, mac_addr);
			       ...
			       const char *buf, size_t count)
{
	...
	struct siena_nic_data *nic_data = efx->nic_data;
	struct efx_local_addr *local_addr;
	u8 new_addr[ETH_ALEN] __aligned(2);
	int rc = 0;
	...
	if (buf[0] != '-' && buf[0] != '+')
		return -EINVAL;
	if (!mac_pton(buf + 1, new_addr))
		return -EINVAL;

	mutex_lock(&nic_data->local_lock);
	list_for_each_entry(local_addr, &nic_data->local_addr_list, link) {
		if (!ether_addr_equal(local_addr->addr, new_addr)) {
			if (buf[0] == '-') {
	...
	if (buf[0] == '+') {
		...
		ether_addr_copy(local_addr->addr, new_addr);
		list_add_tail(&local_addr->link, &nic_data->local_addr_list);
		goto update;
	}
...
{
	struct siena_nic_data *nic_data = container_of(data, struct siena_nic_data, peer_work);
	struct efx_nic *efx = container_of((void *)nic_data, struct efx_nic, nic_data);
...
static void efx_siena_sriov_vf_attrs_fini(struct efx_nic *efx)
{
	struct efx_vf *vf;
	unsigned int pos;
...
int efx_siena_sriov_init(struct efx_nic *efx)
{
	...
	rc = efx_nic_alloc_buffer(efx, &nic_data->vfdi_status, sizeof(*vfdi_status),
				  GFP_KERNEL);
	if (rc)
		goto fail_status;
	 ...
	 */
	efx_siena_sriov_vf_attrs_init(efx);
#endif
...
void efx_siena_sriov_mac_address_changed(struct efx_nic *efx)
{
	...
	if (!efx->vf_init_count)
		return;
	ether_addr_copy(vfdi_status->peers[0].mac_addr, efx->net_dev->dev_addr);
	queue_work(vfdi_workqueue, &nic_data->peer_work);
}
...
int efx_siena_sriov_set_vf_mac(struct net_device *net_dev, int vf_i, u8 *mac)
{
	...
	if (vf_i >= nic_data->vf_rtnl_count)
		return -EINVAL;
	vf = efx->vf + vf_i;
