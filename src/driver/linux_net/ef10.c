...
static int efx_ef10_init_datapath_caps(struct efx_nic *efx)
{
	...
	/* VEB vswitching will be added as a capability in bug43203; until then,
					       ...
					       char *buf)
{
	struct efx_nic *efx = pci_get_drvdata(to_pci_dev(dev));
	return sprintf(buf, "%d\n",
		       ((efx->mcdi->fn_flags) & (1 << MC_CMD_DRV_ATTACH_EXT_OUT_FLAG_LINKCTRL)) ? 1 : 0);
}
					  ...
					  char *buf)
{
	struct efx_nic *efx = pci_get_drvdata(to_pci_dev(dev));
	return sprintf(buf, "%d\n",
		       ((efx->mcdi->fn_flags) & (1 << MC_CMD_DRV_ATTACH_EXT_OUT_FLAG_PRIMARY)) ? 1 : 0);
}
...
static int efx_ef10_probe(struct efx_nic *efx)
{
	...
	if (rc == 0)
		...
		if (rc)
			goto fail3;
		nic_data->workaround_35388 = enabled & MC_CMD_GET_WORKAROUNDS_OUT_BUG35388;
	}
...
#endif
#endif /* HAVE_PHYSFN */
	memcpy(nic_data->port_id, efx->net_dev->perm_addr, ETH_ALEN);
#endif
...
static void efx_ef10_free_piobufs(struct efx_nic *efx)
{
	...
	for (i = 0; i < nic_data->n_piobufs; i++) {
		...
		rc = efx_mcdi_rpc(efx, MC_CMD_FREE_PIOBUF, inbuf, sizeof(inbuf),
				  NULL, 0, NULL);
		WARN_ON(rc && !efx_ef10_hw_unavailable(efx));
	}
...
static void efx_ef10_tx_init(struct efx_tx_queue *tx_queue)
{
	MCDI_DECLARE_BUF(inbuf, MC_CMD_INIT_TXQ_IN_LEN(EFX_MAX_DMAQ_SIZE * 8 /
						       EFX_BUF_SIZE));
	bool csum_offload = tx_queue->csum_offload;
...
static void efx_ef10_tx_write(struct efx_tx_queue *tx_queue)
{
	 ...
	 */
	if (old_write_count - tx_queue->read_count < tx_push_max_fill) {
				    ...
				    const efx_qword_t *event)
{
	...
	unsigned int rx_eth_tag_class, rx_l3_class;
	...
	} else if (unlikely(EFX_QWORD_FIELD(*event, ESF_DZ_RX_IPCKSUM_ERR))) {
...
static int efx_ef10_fini_dmaq(struct efx_nic *efx)
{
	...
	if (!efx_ef10_hw_unavailable(efx)) {
		efx_for_each_channel(channel, efx) {
					...
					const struct efx_filter_spec *spec)
{
	...
	rc = efx_ef10_filter_rx_match_pri(efx->filter_state, spec->match_flags);
	if (rc < 0)
		return rc;
...
static int efx_ef10_filter_table_probe(struct efx_nic *efx)
{
	...
	if (efx_ef10_filter_rx_match_pri(table,
					 EFX_FILTER_MATCH_FLAGS_RFS) < 0) {
		netif_info(efx, probe, efx->net_dev,
...
#ifdef EFX_HAVE_RWSEM_IS_LOCKED
				      ...
				      bool rollback)
{
	...
	if (rc < 0) {
		if (multicast) {
			netif_warn(efx, drv, efx->net_dev,
				   "Multicast mismatch filter insert failed rc=%d\n",
				   rc);
		} else {
			netif_printk(efx, drv, rc == -EPERM ? KERN_DEBUG : KERN_WARNING,
				     efx->net_dev,
				     "Unicast mismatch filter insert failed rc=%d\n",
				     rc);
		}
	} else if (multicast) {
...
static int efx_ef10_vport_filter_insert(struct efx_nic *efx, unsigned vport_id,
...
static int efx_ef10_vport_filter_remove(struct efx_nic *efx, unsigned vport_id,
...
#elif defined(EFX_NOT_UPSTREAM) && defined(EFX_USE_KCOMPAT) && !defined(EFX_HAVE_PHYSFN)
	/* This is duplicated to avoid interactions between unifdef
	 * and the CONFIG check above. */
	if (rc == -EPERM)
		netif_err(efx, drv, efx->net_dev,
			  "Cannot change MAC address; use sfboot to enable mac-spoofing on this interface\n");
#else
...
#if !defined(EFX_USE_KCOMPAT) || defined(EFX_NEED_GET_PHYS_PORT_ID)
static int efx_ef10_sriov_get_phys_port_id(struct efx_nic *efx,
...
#ifdef EFX_NEED_GET_PHYS_PORT_ID
