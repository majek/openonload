...
#else
#define KERNEL_BASE(paca) KERNELBASE
#endif
...
uint64_t *find_syscall_table(void **return_addr)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,28)
/* These kernels don't use a kernel toc to get system_call_common,
 * but add a 32-bit displacement to the PACA directly.
 *
 *  system_call_common:
 *
 ...
 *
 * SPRG3 is the PACA, and the two ORISes add in the address of
 *  system_call_common
 */

    /* Stop gcc complaining - this is used in another #ifdef branch */
    (void)sys_call_entry_offset;
    
#define POWER_CLRRDI_INSN   0x79ac07c4
#define ORIS_SIGNATURE      0x658c0000
#define ORI_SIGNATURE       0x618c0000
    /* Find the clrrdi  */
    for (i= 0; i < (0x80 >> 2); ++i)
    {
        if (pv[i] == POWER_CLRRDI_INSN)
        {
            uint32_t oris, ori;
            TRAMP_DEBUG("Found our clrrdi .. \n");
            oris = pv[i+1];
            ori = pv[i+2];

            if ((oris & 0xFFFF0000) == ORIS_SIGNATURE && 
                (ori & 0xFFFF0000) == ORI_SIGNATURE)
            {
    ...
    {
        TRAMP_DEBUG("Failed to find CLRRDI/ORIS/ORI sequence. \n");
        rv = ERR_PTR(-EIO);
        goto end;
    }

    TRAMP_DEBUG("Found ORIS: 0x%08x \n", 
              sys_call_entry_offset);
    {
        void *p = (void *)((((uint64_t)a_paca) & (((uint64_t)0xFFFFFFFF) << 32)) | 
                           ((uint64_t)sys_call_entry_offset));

        TRAMP_DEBUG("Found system_call_common @ %p \n", p);
        system_call_common = p;
    }
    
#else
    /* An addi from the kernel base .. */

#if LINUX_VERSION_CODE < KERNEL_VERSION(3,0,0)
    /* Older kernels use r12 to load the toc offset for system_call_entry in:
     *
