...
static void ci_team_dump_one(struct ci_team_control* c, int ifindex)
{
  ...
  OO_DEBUG_BONDING(ci_log("%s: TEAM_CMD_PORT_LIST_GET(%d)",
                          __func__, ifindex));
                                   ...
                                   const struct kernel_param *kp)
{
  ...
  if( old_period == 0 && oo_teaming_dump_period != 0 ) {
    ...
    OO_DEBUG_BONDING(ci_log("%s: dump teaming state every %d jiffies.",
                            __func__, oo_teaming_dump_period));
    queue_delayed_work(c->wq, &c->dump_work, oo_teaming_dump_period);
  }
  return 0;
}
static struct kernel_param_ops ci_team_dump_period_ops = {
                               ...
                               int family_id, int grp_id)
{
  ...
  if( c->grp_id != 0 ) {
    OO_DEBUG_BONDING(ci_log("Unregister team family %d "
                            "with netlink group %d",
                            c->family_id, c->grp_id));
    ci_team_unsubscribe(c);
    c->grp_id = 0;
  }
  ...
  if( grp_id != 0 ) {
    OO_DEBUG_BONDING(ci_log("Register team family %d "
                            "with netlink group %d",
                            family_id, grp_id));
    c->grp_id = grp_id;
                             ...
                             const char* mode)
{
  ...
  OO_DEBUG_BONDING(ci_log("%s(ifindex=%d, %s) new_mode=%d", __func__,
  ...
  if( new_mode == CICP_BOND_MODE_BALANCE_XOR ) {
    OO_DEBUG_BONDING(ci_log("Unaccelerated mode %s team %d",
                            mode, team_ifindex));
                              ...
                              bool linkup, bool is_dump)
{
  ...
  if( rc != 0 || !(encap.type & CICP_LLAP_TYPE_SFC) ||
    ...
    OO_DEBUG_BONDING(ci_log("Non-SFC port %d in team %d, not accelerating", 
                                 ...
                                 int enabled)
{
  ...
  if( port_rowid == CICP_BOND_ROW_NEXT_BAD ) {
    OO_DEBUG_BONDING(ci_log("%s: failed to add slave %d to team %d",
                                         ...
                                         const struct nlattr* nla)
{
  ...
  nla_for_each_nested(nla1, nla, rc1) {
    ...
    OO_DEBUG_BONDING(ci_log("Team %d port %d option %s val %d-%p-%s%s%s",
    ...
    switch( option ) {
      ...
      case CI_TEAM_OPTION_BFP_HASH_FUNC:
        ...
        if( removed )
          ...
          OO_DEBUG_BONDING(ci_log("Teaming TX hash modes are not supported "
                                       ...
                                       const struct nlattr* nla)
{
  ...
  nla_for_each_nested(nla1, nla, rc1) {
    ...
    OO_DEBUG_BONDING(ci_log("Port %d in team %d %s%s%s%s",
                            port_ifindex, team_ifindex,
                            is_dump ? "DUMP" : "notify",
                            removed ? " removed" : "",
                            changed ? " changed" : "",
  ...
  if( is_dump ) {
    ...
    OO_DEBUG_BONDING(ci_log("%s: TEAM_CMD_OPTIONS_GET(%d)",
                                 ...
                                 const struct nlmsghdr* nlh)
{
  ...
  if( rowid < 0 ) {
    ...
    if( netdev != NULL ) {
      ...
      cicpos_llap_import(c->cplane, NULL, team_ifindex, netdev->mtu,
                         netdev->flags & IFF_UP,
                         CICP_LLAP_TYPE_BOND, netdev->name, NULL, NULL);
      dev_put(netdev);
  ...
  if( rowid < 0 ) {
    OO_DEBUG_BONDING(ci_log("Can't find or add a row for team "
...
int ci_teaming_init(struct workqueue_struct* wq, cicp_handle_t* cplane)
{
  struct socket* s;
  char buf[CI_TEAM_GENL_REQ_BUFSIZE];
  struct sk_buff* skb;
  ...
  OO_DEBUG_BONDING(ci_log("NETLINK_GENERIC GENL_ID_CTRL/notify "
...
void ci_teaming_fini(cicp_handle_t* cplane)
{
  ...
  flush_work(&c->read_work); /* Read any OPTIONS_GET */
  flush_work(&c->read_work); /* Read any PORT_LIST_GET */
