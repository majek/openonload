...
/* 
 * For the ppc, things are a differnt ...
 *
 * A syscall() will basiclly execute an 'sc' instruction. This will cause an exception
 * which will then execute instructions from the physical interrupt vector address 
 * 0x0000 _0000_0000_0C000. ie the next instruction is picked up from
 * this address above which will conduct a context switch to kernel mode and 
 * addressbase if the call was issued from user mode etc .. then transfer control
 * to the 'do_syscall()' handler, wherever it is on a ppc32/64 system.
 * See .../linux/arch/ppc/kernel/    head.S, misc.S & entry.S and for ppc64
 *     .../linux/arch/ppc64/kernel/  head.S, misc.S & entry.S for reference.
 *
 * Once the location of the 'do_syscall()' handler is found, then a code match
 * routine may be developed to obtain the actual location of the 'sys_call_table'
 * to support the trampoline system as it has been devised to be working for all x86
 * and x86_64 systems.
 *
 * Also on ppc64 systems, there are two such tables. One dedicated for 32bit mode
 * and the other when the caller is in 64bit mode.
 * 
 */
                                      ...
                                      int flags)
{
...
#ifdef __NR_accept4
  {
    ...
    sys_accept4_fn = syscall_table[__NR_accept4];
...
#if defined(__NR_accept)
  {
    ...
    sys_accept_fn = syscall_table[__NR_accept];
...
exit:
  ...
  if( rc == 0 && safe_signals_and_exit )
    efab_signal_process_init(&signal_data);
  return rc;
}
